...
import { incrementLeadCount } from "../../libs/api";
...
const ClientForm: React.FC = () => {
    ...
    const handleSubmit = async (values: FormValues) => {
        ...
        try {
            // 1) Вставляем нового лида
            const { agreement, ...record } = values;
            const { error } = await supabase.from("leads").insert([record]);
            if (error) throw error;

            // 2) Атомарно инкрементим lead_count
            const newCount = await incrementLeadCount();

            // Мгновенное обновление счётчика в UI
            window.dispatchEvent(new CustomEvent<number>("lead_count_updated", { detail: newCount }));
